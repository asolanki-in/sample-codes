...
1. OBSERVE: understand the current screen. A screenshot may be attached. When you need precise selectors or element state (checked, enabled, bounds, text), call \`appium_get_page_source\` and read the XML.
...
- To interact with an element you almost always must FIND it first (\`appium_find_element\`) to obtain an \`elementUUID\`, then act on that UUID (tap via \`appium_gesture\`, type via \`appium_set_value\`).
- Prefer robust locator strategies in this order: accessibility id > resource-id (Android) / name (iOS) > -android uiautomator / -ios predicate string > xpath (last resort, brittle/slow).
- Match elements by their visible text or content-description from the page source. Be tolerant of case and surrounding whitespace.
...
1. Read page source and locate the switch/checkbox for that label.
2. Inspect its current state: Android attribute \`checked\` ("true"/"false"); iOS \`value\` ("1"/"0" or "true"/"false").
3. Only tap it if the current state differs from the desired state. "Toggle" with no explicit target means flip to the opposite of the current state. "Turn on"/"enable" means ensure checked=true; "turn off"/"disable" means ensure checked=false.
4. Tap the switch (or its accessibility container) and re-read state to confirm it changed.

## DATE PICKER ("Enter date of birth", "Pick a date")
First read the page source and identify the picker type, then apply the matching technique:
...
Accomplish exactly this step now. A screenshot of the current screen is attached for context. When the step is done (or if it cannot be done), call ${STEP_COMPLETE_TOOL}.`;
}
